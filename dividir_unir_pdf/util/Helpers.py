...
class Helpers:
    ...
    def centerWindows(self,windows,heights,widths):    
        pantall_ancho = windows.winfo_screenwidth()
        pantall_largo = windows.winfo_screenheight()
        x = int((pantall_ancho/2) - (widths/2))
        y = int((pantall_largo/2) - (heights/2))
        return windows.geometry(f"{widths}x{heights}+{x}+{y}")
